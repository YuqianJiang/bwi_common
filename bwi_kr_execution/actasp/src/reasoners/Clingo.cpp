...
AnswerSet Clingo::filterState(const std::vector<actasp::AnswerSet>& plans, const std::vector<actasp::AspRule>& goals) {
  ...
  for (; fluent != allFluentsSet.end(); ++fluent) {
    ...
    if (std::find(nofilter.begin(), nofilter.end(), namearity.str()) == nofilter.end()) { //can be filtered
  ...
  //add the minimize statement ( eg: :~ pos(x,y,z). [0@1] )
  ...
  for (fluent=allFluentsSet.begin(); fluent != allFluentsSet.end(); ++fluent) {
    if (std::find(fluentsNames.begin(), fluentsNames.end(), fluent->getName()) == fluentsNames.end()) { //new fluent 
      fluentsNames.push_back(fluent->getName()); //mark it has been added already
      fluentsString << ":~ " << fluent->getName() << "(";
      int n = fluent->arity(); int i = 0;
      fluentsArity.push_back(n);
      string commaparam = "";
      for (; i<n; ++i) {
        char param = 'A' + i;
        fluentsString << commaparam << param;
        commaparam = ",";
      }
      fluentsString << "). [0@1]" << endl;
    }
  }
  fluentsString << endl;

  //add goal to fluentString
  fluentsString << "#external query("  << incrementalVar << ")." << endl;
  std::vector<actasp::AspRule> goalRules = goals;
  std::vector<actasp::AspRule>::iterator ruleIt = goalRules.begin();
  for (; ruleIt != goalRules.end(); ++ruleIt) {
    ruleIt->body.push_back(AspFluent("query(n)"));
  }
  fluentsString << aspString(goalRules,incrementalVar) << endl;
  fluentsString << endl;

  //add the option to show fluents and hide plan
  ...
  while (name != fluentsNames.end() && number != fluentsArity.end()) {
    fluentsString << "#show " << *name << "/" << *number << "." << endl;
    ++name; ++number;
  }
  ...
  for (; plan != plans.end(); ++plan) {
    ...
    planString << "#include <iclingo>." << endl;
    std::vector<actasp::AspFluent>::const_iterator planIt = planSet.begin();
    for (; planIt != planSet.end(); ++planIt) {
      planString << planIt->toString() << "." << endl;
    }
    planString << endl;

    //combine this plan with all the fluents stuff created before
    planString << fluentsString.str();

    //make a query that only uses the domain and what I created, not current.asp
    std::list<actasp::AnswerSet> listResult = krQuery(planString.str(),0,plan->maxTimeStep(),"filterState.asp",0,false);

    //add the result of this plan (since there is 1 AnswerSet from the query, I just take the first element)
