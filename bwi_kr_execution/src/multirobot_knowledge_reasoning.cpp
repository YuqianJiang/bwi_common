...
int main(int argc, char **argv) {
  ...
  QueryGenerator *generator = new Clingo4_2("n",queryDir,domainDirectory,actionMapToSet(ActionFactory::actions()),currentFilePath,PLANNER_TIMEOUT);
...
bool updateFluents(bwi_kr_execution::UpdateFluents::Request  &req,
  ...
  uf.request.name = n.getNamespace().substr(2);
  uf.request.fluents = req.fluents;
  ...
  reasoner->setCurrentState(state);

  res.consistent = uf.response.consistent;
...
bool computePlan(bwi_kr_execution::ComputePlan::Request  &req,
  ...
  cp.request.name = n.getNamespace().substr(2);
  cp.request.goal = req.goal;
  
  planClient.call(cp);

  res.plan = cp.response.plan;
